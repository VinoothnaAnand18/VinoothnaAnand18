## Hi there 👋
I'm Methuku Vinoothna Anand 👋

🎓 Final-year Computer Science undergraduate at **Chaitanya Bharathi Institute of Technology (CBIT)**

💡 Passionate about **Software Engineering, Artificial Intelligence, and Full-Stack Development**

🚀 Experienced in building **AI-driven applications** and **scalable web solutions** using Python, Flask, and the MERN stack

🌱 Currently exploring **Deep Learning, Generative AI, and Agentic AI Systems**

---

## 🛠️ Technical Skills

### Programming Languages

`Python` `Java` `JavaScript` `SQL`

### Web Development

`HTML` `CSS` `Bootstrap` `React.js` `Node.js` `Express.js` `Flask` `REST APIs`

### Databases

`MongoDB` `MySQL` `SQLite`

### AI/ML

`Machine Learning` `Computer Vision` `Scikit-learn` `MediaPipe` `Data Analysis`

### Core CS

`Data Structures & Algorithms` `OOP` `DBMS` `Operating Systems` `Computer Networks`

### Tools

`Git` `GitHub` `Jupyter Notebook` `VS Code`

---

## 🚀 Featured Projects

### 🔹 Naari Raksha – AI-Powered Women Safety System

* Developed an AI-powered women safety platform with **SOS alerts, GPS tracking, ML-based risk prediction, and emergency notifications**.

### 🔹 AI-Based Intelligent Video Surveillance Platform

* Built an intelligent surveillance system using **Python, MediaPipe, and Computer Vision**, achieving **90%+ activity classification accuracy**.

### 🔹 MERN eCommerce Platform

* Developed a full-stack eCommerce application featuring **JWT authentication, Razorpay integration, shopping cart, and admin dashboard**.

### 🔹 Hospital Management System

* Built a full-stack healthcare platform for managing **patients, doctors, and appointments** with secure role-based access control.

---

## 💻 Coding Profiles

* Solved **315+ problems** on LeetCode
* Strong foundation in **Data Structures and Algorithms**
* Passionate about solving challenging algorithmic problems

---

## 🌟 Leadership & Involvement

* **External Affairs Incharge**, ELC CBIT
* **Design Head**, Chaitanya Svaasthya

---

## 📫 Connect With Me

📧 **Email:** [mva2k5@gmail.com](mailto:mva2k5@gmail.com)

💼 **LinkedIn:** [www.linkedin.com/in/methukuvinoothnaanand07](http://www.linkedin.com/in/methukuvinoothnaanand07)

💻 **GitHub:** github.com/VinoothnaAnand18

⭐ *Always open to collaborating on impactful software and AI projects.*

<!--
**VinoothnaAnand18/VinoothnaAnand18** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
